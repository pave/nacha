...
class AchFile(object):
    ...
    def add_batch(self, std_ent_cls_code, batch_entries=None,
        ...
        for record in batch_entries:
            ...
            entry.amount = record['amount'] #cents
        ...
        self.batches.append(FileBatch(batch_header, entries, self.debug))
        self.set_control()
    ...
    def enumerate_row(self, row, line_ending):
        ret = row + line_ending
        if not self.debug:
            return ret
        ...
        return ret + line_ending + line_ending
    ...
    def render_to_string(self, force_crlf=False):
        ...
        ret_string = self.enumerate_row(self.header.get_row(), line_ending)

        for batch in self.batches:
            ret_string += batch.render_to_string(force_crlf=force_crlf)

        ret_string += self.enumerate_row(self.control.get_row(), line_ending)
...
class FileBatch(object):
    ...
    def __init__(self, batch_header, entries, debug=False):
        """
        ...
        for entry, addenda in entries:
            ...
            self.entries.append(FileEntry(entry, addenda, self.debug))
    ...
    def render_to_string(self, force_crlf=False):
        ...
        ret_string = self.enumerate_row(self.batch_header.get_row(), line_ending)

        for entry in self.entries:
            ret_string += entry.render_to_string(force_crlf=force_crlf)

        ret_string += self.enumerate_row(self.batch_control.get_row(), line_ending)
...
class FileEntry(object):
    ...
    def __init__(self, entry_detail, addenda_record=[], debug=False):
        """
        ...
        for index, addenda in enumerate(addenda_record, 1):
    ...
    def render_to_string(self, force_crlf=False):
        ...
        ret_string = self.enumerate_row(self.entry_detail.get_row(), line_ending)

        for addenda in self.addenda_record:
            ret_string += self.enumerate_row(addenda.get_row(), line_ending)
